...
function groupType(name: string) {
  ...
  if (["vodka", "rum", "tequila", "gin", "bourbon", "whiskey"].some((k) => n.includes(k))) return "Spirits";
  if (["juice", "ginger beer", "tonic", "soda", "vermouth", "campari", "aperol", "prosecco", "triple sec", "liqueur", "espresso"].some((k) => n.includes(k)))
    return "Mixers";
  if (["mint", "orange", "lime", "lemon", "peel"].some((k) => n.includes(k))) return "Garnish & Citrus";
  if (["bitters"].some((k) => n.includes(k))) return "Bitters";
  return "Other";
}
...
export async function POST(req: Request) {
  ...
  // Choose menu by vibe:
  // We keep it deterministic + curated for conversion (confidence > randomness).
  const vibePick =
    vibe === "classic"
  ...
  // Match each menu drink and union missing ingredients
